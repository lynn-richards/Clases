...
```{r cargando bases}
casos <- data.table(read_excel("COVID_19_up_to_29_02_2020_cases.xlsx"),stringsAsFactors = F) #No me asignes un valotr numerico a la columna
...
cbind(x=casos,y=muertes[,.(deaths)])# Para que no se repitan las columnas, nos quedamos solo con la columna "deaths" de "muertes", el punto sirve para que sea columnas
#Si no estan ordenadas las columnas las pegara igual por eso es mejor merge
```
...
```{r merge}
casosymuertes <- merge(casos,muertes, by = c("day","month","year","countriesAndTerritories"), all = T,sort=F) #all = T --> para pegar las filas  sort = F para no ordenar
```
...
al_29_03_2020[!complete.cases(al_29_03_2020),]  ## Otro ejemplo, buscame  en la base dos, todas las filas qque no esten completas
...
apply(al_29_03_2020, 2, function(col){
  paste0(round(sum(is.na(col))/length(col),6)*100,"%") #tengo un objeto para 
  })
apply(al_29_03_2020, 2, function(col){
  paste0(round(sum(is.na(col))/length(col),5)*100,"%") #tengo un objeto para 
  })
```
...
```{r grepl}
unique(grep("Per",al_29_03_2020_completo[,countriesAndTerritories], value = T)) #Quiero todos los valor bque empiecen con per, si es que no se si tiene tilde o no
```
...
- Perú ahora presenta un tilde en la u.
...
```{r colapsar}
pormes <- chileyvecinos_completa[, .(cases = sum(cases,na.rm = T),deaths = sum(deaths,na.rm = T), popData2018 = round(mean(popData2018, na.rm = T),0)), by = c("month","countriesAndTerritories")] # round(mean(popData2018, na.rm = T),0) me redonde los decimales
```
